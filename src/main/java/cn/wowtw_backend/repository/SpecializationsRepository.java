...
public interface SpecializationsRepository extends JpaRepository<Specializations, Integer> {
    ...
    @Query("SELECT new cn.wowtw_backend.model.infoGroup.SpecializationsDTO(t.id, s.name, s.string) FROM Specializations s JOIN TalentsSpecializations t ON s.id = t.specializations.id WHERE t.talents.id = :talentId")

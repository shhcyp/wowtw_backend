...
public interface LeathersRepository extends JpaRepository<Leathers, Integer> {
    ...
    @Query("SELECT new cn.wowtw_backend.model.infoGroup.GearBaseDTO(t.id, l.icon, l.part, l.name, l.quality, t.isMark, l.drop, t.isExtra) FROM Leathers l JOIN TalentsLeathers t ON l.id = t.leathers.id WHERE t.talents.id = :talentId")
